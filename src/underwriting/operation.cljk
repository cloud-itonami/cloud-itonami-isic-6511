(ns underwriting.operation
  "OperationActor -- one underwriting operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (Underwriter-LLM) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the UnderwritingGovernor (:govern) and the
  ...
  One graph run = one underwriting operation (intake -> advise -> govern
  ...
  decision to a human operator (the licensed underwriter). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:policy/bind` ALWAYS reaches this node when the governor is clean --
  see `underwriting.phase`."
...
(defn build
  ...
  `underwriting.store/Store`).
  opts:
    :advisor      -- an `underwriting.underwriterllm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; UnderwritingGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
