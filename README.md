...
```bash
kbb -M:dev:run     # walk one clean application + one HARD-hold case through the actor
...
| File | Role |
|---|---|
| `src/underwriting/store.cljk` | **Store** protocol -- `MemStore` ‖ `DatomicStore` (`langchain.db`) + append-only audit ledger + policy-binding history |
| `src/underwriting/registry.cljk` | Policy-binding draft records (no fabricated international check-digit standard -- see docstring) |
| `src/underwriting/facts.cljk` | Per-jurisdiction underwriting requirement catalog with an official spec-basis citation per entry, honest coverage reporting |
| `src/underwriting/underwriterllm.cljk` | **Underwriter-LLM Advisor** -- `mock-advisor` ‖ `llm-advisor`; intake/assessment/KYC/binding proposals |
| `src/underwriting/corporate_intel.cljk` | optional cross-reference into [`cloud-itonami-isic-8291`](https://github.com/cloud-itonami/cloud-itonami-isic-8291)'s `:disclosure/screen-name` (ADR-2607110400 §5) -- catches an insured/beneficiary clean on every LOCAL field but flagged in 8291's own sourced PEP/sanctions data; wired into `screen-kyc` via an injected fn, default is a no-op so every prior caller's behavior is unchanged unless explicitly opted in |
| `src/underwriting/governor.cljk` | **UnderwritingGovernor** -- spec-basis · sanctions hold · document-complete · confidence floor · actuation gate |
| `src/underwriting/phase.cljk` | **Phase 0→3** -- read-only → assisted intake → assisted assess/screen → supervised (binding always human) |
| `src/underwriting/operation.cljk` | **OperationActor** -- langgraph-clj StateGraph |
| `src/underwriting/sim.cljk` | demo driver |
