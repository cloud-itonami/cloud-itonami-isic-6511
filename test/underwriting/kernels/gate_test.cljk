(ns underwriting.kernels.gate-test
  ...
  3. governor boundary — the confidence floor boundary and the
     ...
     through the real `underwriting.governor/check` façade."
...
(deftest policy-bind-auto-enabled-nowhere
  (testing "op 4 (:policy/bind) is auto-enabled at NO phase — kernel restates
            the phase table's permanent structural invariant"
...
;; touches neither the store nor the docs/spec checks, so the verdict
;; is decided purely by confidence/actuation — nil store is safe.
